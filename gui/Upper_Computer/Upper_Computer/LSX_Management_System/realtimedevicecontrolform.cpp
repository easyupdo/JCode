...
#include "ui_realtimedevicecontrolform.h"
#include <QFile>
...
RealTimeDeviceControlForm::RealTimeDeviceControlForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RealTimeDeviceControlForm)
{
    ui->setupUi(this);
    ...
    connect(&distensePlayTime,&QTimer::timeout,this,&RealTimeDeviceControlForm::showDistansePlayTime);



    deviceControl_global= new global;
//    deviceControl_global->
    connect(this,&RealTimeDeviceControlForm::toLoginDialogForCheckDB,deviceControl_global,&global::fromUserSqlClassForCheckDB);
    //初始化proto_Message
//    socket = new QTcpSocket;

    msg = new ProtocolMessage;
    connect(this,&RealTimeDeviceControlForm::toProtocolMessage_2,msg,&ProtocolMessage::fromDeviceControl_2);

    //发送给msg
    //realtimedevicecontrolfrom -> protocomessage
    connect(this,&RealTimeDeviceControlForm::toProtocolMessageForSetSocket,msg,&ProtocolMessage::fromProtocolMessageForSetSocket);
    connect(msg,&ProtocolMessage::toRealTimeFormForSocketOK,this,&RealTimeDeviceControlForm::fromRealTimeFormForSocketOK);


    connect(this,SIGNAL(toProtocolMessage(QString)),msg,SLOT(fromDeviceControl(QString)));
    //MD5验证方法
//    connect(realTimeMSG,&ProtocolMessage::toRealTimeForVerifyVoiceRecordMD5,this,&RealTimeWork::fromProtocolMessageForVerifyVoiceRecordMD5,Qt::DirectConnection);
//    connect(this,&RealTimeWork::toProtocolMessageForSocket,realTimeMSG,&ProtocolMessage::socket_Receive);//将socket传递到protocolmesage中

    connect(msg,&ProtocolMessage::toDeviceControl,this,&RealTimeDeviceControlForm::fromProtocolMessage);
    connect(msg,&ProtocolMessage::toDeviceControlNum,this,&RealTimeDeviceControlForm::fromProtocolMessageNum,Qt::DirectConnection);
    connect(msg,&ProtocolMessage::toDeviceControlGetMusicFileLength,this,&RealTimeDeviceControlForm::fromProtocolMessageSentMusicFileLength);
    connect(msg,&ProtocolMessage::toDeviceTellMusicPlayOver,this,&RealTimeDeviceControlForm::fromProtocolMessageTellMusicPlayOver);

    connect(this,&RealTimeDeviceControlForm::toProtocolMessageForPlayMusic,msg,&ProtocolMessage::fromDeviceForTopcm2play);
    connect(this,&RealTimeDeviceControlForm::toProtocolMessageForToPcm2playClearPlayAllBUffer,msg,&ProtocolMessage::fromDeviceForTopcm2playClearAllBuffer);
    connect(this,&RealTimeDeviceControlForm::toProtocolMessageForToPcm2PlayStopPlay,msg,&ProtocolMessage::fromDeviceForToPcm2PlayForStopDistancePlay);
    connect(this,&RealTimeDeviceControlForm::toProtocolMessageForResetCount,msg,&ProtocolMessage::fromDeviceResetCount);
    connect(this,&RealTimeDeviceControlForm::toProtocolMessageForToPcm2playForChangeVolume,msg,&ProtocolMessage::fromDeviceControlForToPcm2playForChangeVolume);
...
//    connect(md,&QItemSelectionModel::selectionChanged,this,&RealTimeDeviceControlForm::SelectStatus);
    //tableview_2添加右键菜单
...
RealTimeDeviceControlForm::~RealTimeDeviceControlForm()
{
    delete ui;
}

/*
void RealTimeDeviceControlForm:: slot_newConnection()
{
...
void RealTimeDeviceControlForm::writeWavHeader(int second,QString fileName)
{ // 开始准备WAV的文件头
    WAVFILEHEADERINFO WavFileHeader;
...
//void RealTimeDeviceControlForm::my_receive()
//{
...
//}
void RealTimeDeviceControlForm::on_pushButton_3_clicked()//download
{
    socket->write(this->msg->M_804C);
}
//********BUG******本地语音目录获取问题 已经修正20170804

void RealTimeDeviceControlForm::on_pushButton_clicked()//connect
{
    ...
    qDebug()<<QString::fromLocal8Bit("是否可以连接")<<connectStatus;
...
//检查网络连接状态
void RealTimeDeviceControlForm::checkSockeConnectSttaus()
{
    qDebug()<<QString::fromLocal8Bit("socket状态:")<<socket->state();
    if(socket->state() == QAbstractSocket::UnconnectedState)
    {
        QMessageBox box;
        box.setText(QString::fromLocal8Bit("设备连接已经断开!"));
        box.exec();
...
void RealTimeDeviceControlForm::nowConnectStatus()
{
    ...
    if(socket->state() == QAbstractSocket::ConnectedState)//如果已经连接到 则开始发送心跳包 保活
    {
        ...
        QMessageBox box;
        box.setText(QString::fromLocal8Bit("设备已连接！"));
        int status = box.exec();
...
//            checkSockeConnectSttaus();
    ...
    {
        box.setUserMessage(QString::fromLocal8Bit("网络未连接,请检查设备"),"error");
        int ret = box.exec();
        if(ret)
        {
            connectStatusTimer->stop();
...
void RealTimeDeviceControlForm::reConnectSocekt()//重连记录仪
{
...
//             socket->connectToHost(QHostAddress(connectIp),connectPort);
    ...
    }else//超时连接 终止定时器
    {
        ...
        ui->pushButton->setText(QString::fromLocal8Bit("连接"));
        QMessageBox box;
        box.setText(QString::fromLocal8Bit("设备连接超时!请检查设备连接情况"));
        box.exec();
    }
...
void RealTimeDeviceControlForm::on_pushButton_4_clicked()//voicePalyer
{
//  setLocalVoiceValue();
...
void RealTimeDeviceControlForm::getVoiceDirFile(QString dirPath)//
{
...
void RealTimeDeviceControlForm::deviceControlSelectDevice(QString sql,QString currentSelectData,QComboBox *comBox,int indexValue)
{
...
void RealTimeDeviceControlForm::getMusicFile(QString musicPath)
{
...
//1 duan >> jian
void RealTimeDeviceControlForm::on_comboBox_activated(const QString &arg1)
{
    musicSelectStatus = false;
//    updateTableViewShow();
...
//2 jian >> qu
void RealTimeDeviceControlForm::on_comboBox_2_activated(const QString &arg1)
{
//    updateTableViewShow();
    musicSelectStatus = false;
    ui->comboBox_3->clear();
...
//3 qu >> sation
void RealTimeDeviceControlForm::on_comboBox_3_activated(const QString &arg1)
{
//    updateTableViewShow();
    musicSelectStatus = false;
    ui->comboBox_4->clear();
...
// station >> device
void RealTimeDeviceControlForm::on_comboBox_4_activated(const QString &arg1)
{
//    updateTableViewShow();
    musicSelectStatus = false;
    ui->comboBox_5->clear();
...
void RealTimeDeviceControlForm::on_comboBox_5_activated(const QString &arg1)
{
//    updateTableViewShow();
    musicSelectStatus = false;
    {
...
void RealTimeDeviceControlForm::initComboBox()
{
    QSqlQuery query;
...
void RealTimeDeviceControlForm::setInitStyle()
{
...
void RealTimeDeviceControlForm::setInitStyleTableview()
{
...
//    tableviewModel->setHeaderData(3,Qt::Horizontal,QString::fromLocal8Bit("文件名"));
    ...
    qDebug()<<QString::fromLocal8Bit("为什么开启调听ui后 ui显示不到tableview");

}



void RealTimeDeviceControlForm::on_verticalSlider_sliderMoved(int position)//Min0 - Max100
{
...
void RealTimeDeviceControlForm::on_pushButton_5_clicked()
{
  voicePlayer->stop();
...
void RealTimeDeviceControlForm::showNowPosition()//实时显示位置
{
    if(playType)//远程
    {
    }
...
void RealTimeDeviceControlForm::setMusicTime(qint64 dx,qint64 px)//讲音频文件长度转化为时分秒制
{
    qint64 time=dx;
...
void RealTimeDeviceControlForm::on_radioButton_clicked(bool checked)//循环
{
...
void RealTimeDeviceControlForm::on_pushButton_6_clicked()
{
...
void RealTimeDeviceControlForm::on_tableView_2_clicked(const QModelIndex &index)
{
...
void RealTimeDeviceControlForm::on_pushButton_2_clicked()//802c
{
    if(!connectStatus)
    {
        iMessageBox box;
    ...
    {
        qDebug()<<QString::fromLocal8Bit("socket状态:")<<this->socket->state()<<this->socket;
        socket->write(this->msg->M_802C);
    }
...
void RealTimeDeviceControlForm::fromMainWindow(QString string)
{
...
//将解析后的数据填充到tableview
void RealTimeDeviceControlForm::fromProtocolMessage(QString print_Start_Time,QString print_EndTime,QString print_Length,int record_Num)
{
    qDebug()<<QString::fromLocal8Bit("填充tableviewHAAAAAAAAAAAAAAA")<<print_Length;
    record_PrintCount +=1;
...
void RealTimeDeviceControlForm::fromProtocolMessageNum(int record_Num)
{
    qDebug()<<QString::fromLocal8Bit("收到查询到多少条信号");
    record_AllNum =record_Num;
    ui->label_16->setText(QString::number(record_Num));
}

void RealTimeDeviceControlForm::PLAYMUSIC()
{
    playAndPauseControl = false;
    showDistansePlayTimeVariable = 0;
    playType = DISTENSE;
...
void RealTimeDeviceControlForm::DOWNLOADMUSIC()
{
...
//void RealTimeDeviceControlForm::SelectStatus(const QItemSelection &selected, const QItemSelection &deselected)
//{
...
void RealTimeDeviceControlForm::on_tableView_pressed(const QModelIndex &index)
{
...
void RealTimeDeviceControlForm::fromProtocolMessageSentMusicFileLength(int second)
{
...
//远程播放的时候的进度条指示
void RealTimeDeviceControlForm::showDistansePlayTime()
{
...
void RealTimeDeviceControlForm::on_tableView_doubleClicked(const QModelIndex &index)
{
...
//    musicSelectStatus = false;
    playAndPauseControl = false;
    QMessageBox box;
    box.setText(QString::fromLocal8Bit("播放语音!"));
...
//    box.buttonRole(1);
    ...
    if(boxStatus == QMessageBox::Ok)
        this->PLAYMUSIC();
    else
        box.close();
}

void RealTimeDeviceControlForm::on_tableView_clicked(const QModelIndex &index)
{
    musicSelectStatus = true;
}

void RealTimeDeviceControlForm::updateTableViewShow()
{
...
void RealTimeDeviceControlForm::on_tableView_2_doubleClicked(const QModelIndex &index)
{
...
void RealTimeDeviceControlForm::setLocalVoiceValue()
{
...
void RealTimeDeviceControlForm::fromProtocolMessageTellMusicPlayOver()
{
    qDebug()<<"Signale from ProtocolMessage!";
    distanceOneMusicPlayOver = true;
    musicSelectStatus = false;
}
//void RealTimeDeviceControlForm::rebootSetVolume()
//{
//    this->readINI();
//}

//void RealTimeDeviceControlForm::shutDownAppRecodVolume()
//{
//    this->writeINI();
//}

//每次关闭记录音量大小 重启读取配置
void RealTimeDeviceControlForm::readINIForRebootSetVolume()
{
...
void RealTimeDeviceControlForm::writeINIForShutDownAppRecodVolume(int voiceVolume)
{
    QString AppPath = QCoreApplication::applicationDirPath();
    QString configIniFile = AppPath + "/config" + "/config.ini";
...
void RealTimeDeviceControlForm::fromMainWindowForSaveInfo()
{
    writeINIForShutDownAppRecodVolume(ui->verticalSlider->value());
}


void RealTimeDeviceControlForm::fromDevicePollingForm(QTcpSocket * socket,QString deviceInfo[8])
{
    this->socket = socket;
    qDebug()<<QString::fromLocal8Bit("收到devicePollingFrom信号")<<this->socket->state()<<this->socket;

    ui->comboBox->clear();ui->comboBox->addItem(deviceInfo[0]);
    ui->comboBox_2->clear();ui->comboBox_2->addItem(deviceInfo[1]);
    ui->comboBox_3->clear();ui->comboBox_3->addItem(deviceInfo[2]);
    ui->comboBox_4->clear();ui->comboBox_4->addItem(deviceInfo[3]);
    ui->comboBox_5->clear();ui->comboBox_5->addItem(deviceInfo[4]);
    tableviewModel->clear();//清数据源
    setInitStyleTableview();//设置表头
    ui->label_16->setText("0");
    setThisUISocket();//socket指针指向存入map中的socket
//    on_pushButton_clicked();
    connectStatus = true;
    connectStatusTimer->start(1000);
...
void RealTimeDeviceControlForm::setThisUISocket()
{
    qDebug()<<QString::fromLocal8Bit("赋值socket");
    msg->setSocket(socket);//msg对象中设置socket
    qDebug()<<QString::fromLocal8Bit("绑定socket和msg对象");
    connect(socket,&QTcpSocket::readyRead,msg,&ProtocolMessage::my_Receive);//协议中接受
//    connect(socket,SIGNAL(readyRead()),msg,SLOT(my_Receive()));//协议中接受 //将socket与接收函数连接
}
...
void RealTimeDeviceControlForm::on_addDuan_CloseButton_clicked()
{
//    connectStatusTimer = new QTimer;
//    disconnect()
...
void RealTimeDeviceControlForm::mouseMoveEvent(QMouseEvent *e)
{
...
void RealTimeDeviceControlForm::mousePressEvent(QMouseEvent *e)
{
...
void RealTimeDeviceControlForm::mouseReleaseEvent(QMouseEvent *)
{
    mousePressed = false;
}
...
void RealTimeDeviceControlForm::fromProtocolMessageForVerifyVoiceRecordMD5()
{
...
void RealTimeDeviceControlForm::fromDevicePollingFormForGotProtocolMessage(QTcpSocket * socket)
{
    qDebug()<<QString::fromLocal8Bit("收到第二步信号");
    this->socket = socket;//为了兼容老版本架构
    emit toProtocolMessageForSetSocket(socket);
}

//接收protocolmessage通知socket赋值完成 可以发送800信号
void RealTimeDeviceControlForm::fromRealTimeFormForSocketOK()
{

    //发送800信号
    qDebug()<<QString::fromLocal8Bit("服务器发送MD5验证记录仪");
    this->socket->write(msg->M_800C);//md5认证
//    qDebug()<<QString::fromLocal8Bit("socket地址")<<socket;
}
