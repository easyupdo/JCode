...
#else
typedef LONGLONG           vgetty_s_int64;
typedef unsigned long      vgetty_u_int64;
#endif
...
void wavtorockwell_init(void) {
    int w;
    /* Reset the coefficient table for the pole and zero linear predictor. */
    for (w = 0; w < 8; w++) RV_pzTable[w] = 0;
    RV_Reset(4);//(nbits);
    RV_cd = RV_cdx[2];//[nbits-2];
    RV_cd_length = RV_cdx_length[2];//[nbits-2];
}

unsigned char wavtorockwell(short dat) {
    unsigned char ret = (unsigned char)RV_ComOne(dat);
    return ret & 0x0F;
}

short rockwelltowav(unsigned char dat) {
    unsigned char c = dat & 0x0F;
    return RV_DecomOne(RV_mul_p[c], RV_Zeta_p[c]);
}
