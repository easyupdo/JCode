...
//    setDufaultVoiceDownDir();//设置默认语音下载目录
   ...
   // QCryptographicHash * newKey = new QCryptographicHash;
    ...
    MD5 standardMD5(text.toStdString());
    QString standardMD532 = QString::fromStdString(standardMD5.md5());
    QString standardMD516 = standardMD532.mid(8,16);

    qDebug()<<QString::fromLocal8Bit("标准MD532:")<<standardMD532;
    qDebug()<<QString::fromLocal8Bit("标准MD516：")<<standardMD516;
...
void ProtocolMessage::msg_Analysis(QByteArray &arec)
{
    ...
    if(r_id == 801)
    {
        ...
        QByteArray serverDigest = arec.mid(sizeof(int)*4,16);
        int voiceRecordTextLength = byteHexToInt(arec,32);//第6个字段 语音记录仪明文长度
        QByteArray voiceRecordText = arec.mid(36,128);

        qDebug()<<QString::fromLocal8Bit("解析801-语音记录仪返回的验证信息:")<<serverDigestLength<<serverDigest.toHex()<<voiceRecordTextLength<<voiceRecordText;
        emit toMainShowMD5(serverDigest.toHex(),800);
        emit toMainShowMD5(QString::number(voiceRecordTextLength),001);
        emit toMainShowMD5(voiceRecordText.toHex(),001);
        ...
        if(ret)
        {
            ...
            emit toMainShowMD5(QString::fromLocal8Bit("上位机MD5验证通过"),002);
        }
//        system("pause");
...
{
    M_804C = intToByteHex(20)+intToByteHex(0)+intToByteHex(804)+intToByteHex(1)+intToByteHex(position);
...
void ProtocolMessage::setCmd801C(QByteArray voiceRecordText)
{
    ...
    {
        //使用md5源文件生成md5
        //QString voiceRecordStandarMD5Result32 = getStandardMD5Result(voiceRecordText);
        //QString voiceRecordStandarMD5Result16 = voiceRecordStandarMD5Result32.mid(8,16);//16位长
        //

    }
    QByteArray QtMD532 =  QCryptographicHash::hash(voiceRecordText,QCryptographicHash::Md5);
    M_801C = intToByteHex(53)+intToByteHex(0)+intToByteHex(801)+intToByteHex(17)+nowTime.toLatin1()+intToByteHex(16)+QtMD532;
    qDebug()<<QString::fromLocal8Bit("M801C消息:")<<M_801C.toHex();
    qDebug()<<QString::fromLocal8Bit("M801C消息:")<<M_801C;

    qDebug()<<QString::fromLocal8Bit("CCCCCCCCCCCCCCCCCC")<<QtMD532;



    emit toMainShowMD5(QtMD532.toHex(),801);
...
QString ProtocolMessage::getCustomMD5Result(QString Text, uint4 key[])
{
    MD5 serverMD5(Text.toStdString(),key);
    return QString::fromStdString(serverMD5.md5());
}

QString ProtocolMessage::getStandardMD5Result(QString Text)
{
    MD5 serverMD5(Text.toStdString());
    return QString::fromStdString(serverMD5.md5());
}
...
{
    qDebug()<<QString::fromLocal8Bit("记录仪MD5")<<skeyMD5;
    if(localMD5 == skeyMD5)
        return 1;
...
//    qDebug()<<QString::fromLocal8Bit("服务器标准MD5_32:")<<localSkeyMD5_32;
//    QString localSkeyMD5_16 = localSkeyMD5_32.mid(8,16);//16位长
//    qDebug()<<QString::fromLocal8Bit("服务器标准MD516:")<<localSkeyMD5_16;
    }
    QByteArray QtMD532 =  QCryptographicHash::hash(text.toLatin1(),QCryptographicHash::Md5);
    return QtMD532;
}
